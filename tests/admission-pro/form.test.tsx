...
describe("invitation-only frontend", () => {
  ...
  it("keeps token and PII out of storage and address bar, prefills without retyping, permits optional editing", async () => {
    ...
    fireEvent.change(document.querySelector("#required_schedule")!, { target: { value: "Mañanas" } });
    ...
    expect((document.querySelector("#required_schedule") as HTMLInputElement).value).toBe("Mañanas");
    fireEvent.click(screen.getByRole("button", { name: "Continuar" }));
  ...
  it("editing after an error preserves the draft and creates a changed attempt with the same invitation", async () => {
    ...
    fireEvent.change(document.querySelector("#required_schedule")!, { target: { value: "TEST Cambio de horario" } });
    ...
    expect(edited.token).toBe(original.token); expect(edited.answers.required_schedule).toBe("TEST Cambio de horario");
